...
function AccountIndicator({ user }) {
  ...
  return (
    <div className="navbar-account anon" title="Sign in">
      <PersonIcon />
    </div>
  )
}
  ...
  return (
    <>
      <nav className="navbar">
        {/* Logo */}
        <div className="navbar-logo" style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          {svgContent ? (
            <span
        ...
        {/* Subject name */}
        {appConfig?.subjectTitle && (
          <div className="navbar-subject">{appConfig.subjectTitle}</div>
        )}
        ...
        <div className="navbar-items navbar-desktop-only">
          {VIEWS.map(v => (
            <div
              key={v.id}
              className={`navbar-item ${currentView === v.id ? 'active' : ''}`}
        ...
        {/* Mobile hamburger */}
        <div
          className={`navbar-hamburger navbar-mobile-only ${menuOpen ? 'open' : ''}`}
      ...
      <div className={`navbar-menu navbar-mobile-only ${menuOpen ? 'open' : ''}`}>
        <div className="navbar-menu-inner">
          {VIEWS.map(v => (
            <div
              key={v.id}
              className={`navbar-menu-item ${currentView === v.id ? 'active' : ''}`}
              onClick={() => handleNavigate(v.id)}
            >
